...
The collector is implemented as the .NET 10 `SolastaBot.ChainCollector` project.

```powershell
dotnet build SolastaBot.slnx -c Release
dotnet run --project SolastaBot.Tests -c Release
dotnet run --project SolastaBot.ChainCollector -c Release -- data/chain
```
...
during the research spike; 4,979 sequential requests at a 0.9 s gap completed with zero failures.

One global adaptive rate limiter governs every HTTP request. It halves its rate and pauses for
30 seconds after a `429` or server-side failure, then gradually recovers toward the configured
0.80 requests/second limit.

Do not run two collectors against the same IP unless the combined request rate is accounted for.

## Running it for real

The cost of this approach is calendar time, so the collector is better suited to a VPS or another
machine that remains online continuously.

Build once:

```bash
dotnet build SolastaBot.ChainCollector/SolastaBot.ChainCollector.csproj -c Release
```

Then run the compiled collector:

```bash
dotnet SolastaBot.ChainCollector/bin/Release/net10.0/SolastaBot.ChainCollector.dll /var/lib/chain-collector
```

For a simple detached Linux process:

```bash
nohup dotnet SolastaBot.ChainCollector/bin/Release/net10.0/SolastaBot.ChainCollector.dll /var/lib/chain-collector > collector.log 2>&1 &
```

Both streams are append-only and flushed per line, so stopping the process preserves completed
history. A restart loses only the mints that were actively being tracked at shutdown.

The status line reports launches, estimated launches/day, tracked arms, currently active mints,
completed polls, feed and poll errors, active-cap drops, and the current adaptive request rate.

`full=` counts launches that qualified for tracking but were dropped because the active-mint cap was
already reached. A rising value means the admission rates, polling schedule, request budget, or
active cap need to be reconsidered.
...
realistic exit rule, a loss distribution, and expectancy at 1, 5 and 30 seconds.

The filter, cohort construction and cost model are already done and reusable; only the return column
is missing.