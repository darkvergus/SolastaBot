...
{
    [Theory]
    [InlineData("pump-sell.json", "6EF8rrecthR5Dkzon8Nwu78hRvfCKubJ14M5uBEwF6P", 2)]
    [InlineData("pump-swap.json", "pAMMBay6oceH9fJKBRHGP5D4bD4sWpmSwMn52FMfXEA", 3)]
    ...
    public async Task PumpSellMatchesActualDevnetInstructionIncludingUpgradeAccounts()
    {
        ...
        string[] keys = recorded.GetProperty("accounts").EnumerateArray().Select(key => key.GetString()!).ToArray();
    ...
    public async Task CompletedCurveResolvesCanonicalPoolAndBuildsBoundedSwapWithUnwrap()
    {
        ...
        TransactionInstruction sale = Assert.Single(routes.Instructions(route, new("sell", keys[3], OrderSide.Sell, 1000, 0, "Exit"), keys[1], 10), instruction => instruction.ProgramId.SequenceEqual(SolanaPrograms.Key(SolanaPrograms.Amm)));
    ...
    public async Task TamperedMintAuthorityIsRefusedBeforeInstructionConstruction()
    {
        ...
        rpc.Accounts[mint] = JsonSerializer.SerializeToElement(new { owner = original.GetProperty("owner").GetString(), executable = false, lamports = 10000000, data = new[] { Convert.ToBase64String(bytes), "base64" } });
    ...
    {
        RouteSnapshot route = new("mint", SolanaPrograms.Token, "Pump", 1, 1_000_000, 1_000_000, 1_000_000, 1_000_000, [100], new Dictionary<string, string>(), SolanaPrograms.System);
        ...
        Assert.True((route with { FeeRates = new BigInteger[] { 200 } }).BuyOutput(10000, 0) < route.BuyOutput(10000, 0));
    }

    [Theory]
    [InlineData("pump", "FFE966C42F1AF41652EE753FE2F1E3F7CD4077D7E6F49FAF3138959C8B56064B")]
