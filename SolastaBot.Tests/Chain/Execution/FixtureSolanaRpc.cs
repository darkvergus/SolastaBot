...
internal sealed class FixtureSolanaRpc : ISolanaRpc
{
    ...
    public FixtureSolanaRpc(string file)
    {
        ...
        for (int index = 0; index < keys.GetArrayLength(); index++) Accounts[keys[index].GetString()!] = values[index];
    }

    public Task<JsonElement> CallAsync(string method, object[] parameters, CancellationToken cancellationToken)
    {
        if (method == "getLatestBlockhash") return Task.FromResult(JsonSerializer.SerializeToElement(new { value = new { blockhash = "11111111111111111111111111111111", lastValidBlockHeight = 1000 } }));
        if (method == "getFeeForMessage") return Task.FromResult(JsonSerializer.SerializeToElement(new { value = 5000 }));
        if (method != "getMultipleAccounts") throw new InvalidOperationException($"Unexpected method {method}");
        string[] addresses = (string[])parameters[0];
        Reads.Add(addresses);
        object?[] values = addresses.Select(address => Accounts.TryGetValue(address, out JsonElement value) ? (object)value : null).ToArray();
